...
using namespace apdos::kernel::actor::connecters;
...
void Actor_Connecter::process_data(std::vector<char> &recv_data, size_t recv_size) {
  if ((read_index + recv_size) > READ_BUFFER_SIZE) {
    std::cout << "Read failed. recv buffer is Full" << std::endl;
    return;
  }

  std::copy(recv_data.begin(), recv_data.begin() + recv_size, recv_buffer.begin() + read_index);
  read_index += recv_size;
